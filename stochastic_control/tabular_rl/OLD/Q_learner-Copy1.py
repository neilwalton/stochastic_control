...
class Q_function(defaultdict):
    ...
    Methods:
        ...
        Q.max(state), Q.min(state) 
            -- gives maximum/minimum value for state
            
        Q.argmax(state), Q.argmin(state)
            -- gives the maximizing/minimizing action
    ...
    def __init__(self,states=None,actions=None):
        # give Q[x][a] structure also count number of udpdates
        super(Q_function, self).__init__(defaultdict(float))
    ...
    def add(self,state,action=None,value=None):  
        ...
        # check is state and action exist 
...
class Q_learning(Q_function):
    ...
    def act(self,state,explore_prob=0.,actions=None):
        '''
